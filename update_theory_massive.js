...
for (const [topic, theory] of Object.entries(massiveTheories)) {
  const safeTheory = theory.replace(/\\/g, '\\\\').replace(/\`/g, '\\`').replace(/\$/g, '$$$$');
  const regex = new RegExp("('" + topic + "'\\s*:\\s*{[\\s\\S]*?inDepthTheory\\s*:\\s*\\`)[\\s\\S]*?(\\`\\s*\\n\\s*\\})", "g");
  
  if (regex.test(fsStr)) {
    fsStr = fsStr.replace(regex, "$1" + safeTheory + "$2");
  }
}

fs.writeFileSync('./src/data/topicContentDict.ts', fsStr);
console.log('Successfully expanded inDepthTheories!');
